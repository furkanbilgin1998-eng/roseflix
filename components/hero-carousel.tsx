...
export function HeroCarousel({slides}:{slides:HeroSlide[]}){
  ...
  return <section className="grain relative min-h-[82vh] overflow-hidden border-b border-white/10">
    ...
    <div className="container-main relative flex min-h-[82vh] items-center justify-center pb-16 pt-24 text-center"><div key={slide.id} className="mx-auto max-w-4xl">
      ...
      <form action="/search" className="hero-actions mx-auto mt-8 flex max-w-xl items-center border border-white/20 bg-black/45 p-1.5 backdrop-blur-xl"><Search className="ml-3 text-muted" size={18}/><LocalizedSearchInput className="min-w-0 flex-1 bg-transparent px-3 py-3 text-sm outline-none"/><button className="bg-rose px-5 py-3 text-xs font-extrabold uppercase tracking-widest">{language==="tr"?"Keşfet":"Discover"}</button></form>
